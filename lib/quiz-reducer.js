 ...
 * @property {Step} step
 * @property {number} quizIndex 0–11 affinity; 12–15 character; 16 pillar; 17 colour
 ...
 * @property {string|null} hatedColor
 */
...
export function loadPersistedState() {
  ...
  try {
    ...
    return {
      ...
      hatedColor: KIT_FAMILIES.includes(parsed.hatedColor)
        ? parsed.hatedColor
        : null,
    };
  } catch {
...
export function quizReducer(state, action) {
  switch (action.type) {
    ...
    case "ANSWER_COLOR": {
      const hatedColor = /** @type {string} */ (action.payload);
      if (!KIT_FAMILIES.includes(hatedColor)) return state;
      return { ...state, hatedColor, step: "complete" };
    }
    case "BACK": {
