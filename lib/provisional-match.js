 ...
 * @param {{ scores: (number|null)[]; pillar: object|null; ownedSlugs: string[]; hatedColor?: string|null }} state
 * @param {object[]} clubs
...
export function provisionalDestination(state, clubs) {
  ...
  const pool = state.hatedColor
    ? clubs.filter((c) => c.kit_family !== state.hatedColor)
    : clubs;
